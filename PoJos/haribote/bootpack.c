	...
	*((int *) 0x0fe8) = (int) nihongo;
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			sprintf(s, "%d-%d-%d %s", get_year(), get_mon_hex(), get_day_of_month(), days[get_day_of_week()]);
			//putfonts8_asc_sht(sht_back, binfo->scrnx - 130, binfo->scrny -30, 0x00000000, 0x00F5F5F5, s, 5);
			putfonts8_asc_sht(sht_back, binfo->scrnx - 180, binfo->scrny -20, COL8_000000, 0X00F5F5F5, s, 15);
			sprintf(s, "%d:%d", get_hour_hex(), get_min_hex());
			putfonts8_asc_sht(sht_back, binfo->scrnx - 45, binfo->scrny -20, 0x00000000, 0x00F5F5F5, s, 5);
