...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, int memtotal)
{
	if (strcmp(cmdline, "mem") == 0 && cons->sht != 0) {
		...
		cmd_exit(cons, fat);
	} else if (strcmp(cmdline, "about") == 0 && cons->sht != 0) {
		cmd_about(cons);
		...
		if (cmd_app(cons, fat, cmdline) == 0) {
			...
			cons_putstr0(cons, "您输入命令的既不是Pojos内部指令，也不是外部程序。\n\n");
		}
	}
	return;
}
...
{
	cons_putstr0(cons, "\n");
	cons_putstr0(cons, "PoJos-os,ver1.0.0-pre1\n");
	cons_putstr0(cons, "Copyright (C) 2018-2020 PanJ\n");
	cons_putstr0(cons, "测试版\n\n");
	return;
}
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
	...
	if (finfo != 0) {
		...
		if (appsiz >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
			...
			cons_putstr0(cons, "这个 *.hrb 文件格式不符合规范，不能打开.\n");
		}
